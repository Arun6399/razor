...
// JavaScript URL AFTER LOGIN
 ...
 function admin_redirect($url, $refresh = 'refresh') {
 	redirect('cpm_admin/'.$url, $refresh);
 }
  // User URL redirect
 function front_redirect($url, $refresh = 'refresh') {
 	//redirect('cpm_front/'.$url, $refresh);
...
function getUserEmail($id='')
{
	if($id!='')
	{
		...
		if ($userDetails->num_rows() > 0)
		{
			$user1=getUserDetails($id,'cpm_email');
			$user=$userDetails->row();
			$email=decryptIt($user->cpm_type).$user1;
		}
		else
		{
			$email='';
...
{
	$ci =& get_instance();
	$number = $ci->db->where('cpm_phone', $phone)->get('users')->row();
	return $number;
}
function phone_check_verified($phone) // full currency row
{
	$ci =& get_instance();
    $ci->db->where('cpm_phone',$phone);
...
function country_location()
{
	$ci =& get_instance();
	$ip =  $_SERVER['REMOTE_ADDR'];
	$details =  json_decode(file_get_contents("http://www.geoplugin.net/json.gp?ip=".$ip));
	$country_row = $ci->common_model->getTableData('countries',array('country_name'=>$details->geoplugin_countryName))->row();
...
function checkEmailfun($email,$password='')
{
	...
	$ci->db->where('history.cpm_type',encryptIt($str1));
...
function checkpair_by_currency($currency_id){
	...
	$ci =& get_instance();
	$pair_currency =  $ci->common_model->customQuery("select * from cpm_trade_pairs where status='1' and  to_symbol_id = ".$currency_id."  order by id DESC")->result();
	if($pair_currency){
		$status =1;
	}else{
		$status =0;
...
function checkpair_currency($currency_id){
	...
	$ci =& get_instance();
	$pair_currency =  $ci->common_model->customQuery("select * from cpm_trade_pairs where status='1' and  to_symbol_id = ".$currency_id." or from_symbol_id = ".$currency_id."  order by id DESC")->row();
	if($pair_currency){
...
function ethers_addressupdate($id,$address,$key,$iv_config)
{

	$ci =& get_instance();
	$web3wallet = $ci->common_model->getTableData('ethers_wallet',array('user_id'=>$id,'type'=>'web3'))->row();
	...
	$insert = $ci->common_model->insertTableData('ethers_wallet', $insert_arr);
	if($insert){
...
function tradable_balance($user_id,$cur_currency,$sec_currency='')
{
	...
	$currency = $ci->common_model->getleftJoinedTableData('currency as a',$hisjoins,$hiswhere,"a.*,b.from_symbol_id,b.buy_rate_value, (SELECT Price FROM `cpm_coin_order` WHERE `pair` = b.id AND `status` IN('filled') ORDER BY `trade_id` DESC LIMIT 1) as Price",'','','','','')->result();
	$btc_amount = 0;
...
{
	$ci =& get_instance();
	$query = $ci->common_model->customQuery("select * from cpm_coin_order where pair='".$pair."' and status = 'filled' order by trade_id desc limit 0,1");
    $pair_detail = $ci->common_model->getTableData('trade_pairs',array('id' => $pair))->row();
	$from_currency = $ci->common_model->getTableData('currency',array('id' => $pair_detail->from_symbol_id))->row();
	$to_currency = $ci->common_model->getTableData('currency',array('id' => $pair_detail->to_symbol_id))->row();
	$pair_symbol = $from_currency->currency_symbol.$to_currency->currency_symbol;
...
{
	$ci =& get_instance();
	$query = $ci->common_model->customQuery("select MAX(price) as high_price from cpm_coin_order where pair ='".$pair."' and status = 'filled' and datetime > DATE_SUB(CURDATE(), INTERVAL 1 DAY)");
...
{
	$ci =& get_instance();
	$query = $ci->common_model->customQuery("select MIN(price) as low_price from cpm_coin_order where pair ='".$pair."' and status = 'filled' and datetime > DATE_SUB(CURDATE(), INTERVAL 1 DAY)");
...
{
	$ci =& get_instance();
	$query = $ci->common_model->customQuery("select sum(Amount) as volume from cpm_coin_order where pair ='".$pair."' and status = 'filled' and datetime > DATE_SUB(CURDATE(), INTERVAL 1 DAY)");
...
function pricechangepercent($pair)
{
	  ...
	  $get_24_data_buy = $ci->common_model->customQuery("select SUM(price) as buy_price from cpm_coin_order where pair ='".$pair."' and datetime > DATE_SUB(CURDATE(), INTERVAL 1 DAY) and type='buy' and status = 'filled' ")->row();

	  $get_details_buy = $ci->common_model->customQuery("select * from cpm_coin_order where pair ='".$pair."' and type='buy' and status = 'filled' order by trade_id desc limit 0,1")->row();

	  $get_24_data_sell = $ci->common_model->customQuery("select SUM(price) as sell_price from cpm_coin_order where pair ='".$pair."' and datetime > DATE_SUB(CURDATE(), INTERVAL 1 DAY) and type='sell' and status = 'filled' ")->row();

	  $get_details_sell = $ci->common_model->customQuery("select * from cpm_coin_order where pair ='".$pair."' and type='sell' and status = 'filled' order by trade_id desc limit 0,1")->row();
...
function checkapi($pair)
{
	...
	$ci =& get_instance();
	$query = $ci->common_model->customQuery("select * from cpm_trade_pairs where id ='".$pair."' and status = 1");
	$result = $query->row();
...
function change_high($pair){
	$ci =& get_instance();
	$query = $ci->common_model->customQuery("select MAX(Price) as high from cpm_coin_order where pair ='".$pair."' and status = 'filled' and datetime > DATE_SUB(CURDATE(), INTERVAL 1 DAY)");
	 $row = $query->row();
	$high = $row->high;
...
function change_low($pair){
	$ci =& get_instance();
	$query = $ci->common_model->customQuery("select MIN(Price) as low from cpm_coin_order where pair ='".$pair."' and status = 'filled' and datetime > DATE_SUB(CURDATE(), INTERVAL 1 DAY)");
	 $row = $query->row();
	$low = $row->low;
	return $low;
}
...
function ethers($method='',$column='',$data='')
{
...
$ch = curl_init();
        ...
        curl_setopt($ch, CURLOPT_URL, "http://127.0.0.1:4009");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 
...
function coinpayments_api_call($cmd, $coin='',$req = array()) {
    ...
    $public_key = '4636cfafe08e2c0d8d53e3a10b7c07716c7d7ca27f0203ca6d1f7b71bd52ee34';
    $private_key = 'B2aCe388BC199F613176B86B4b816Ea2989676A15e39bab9e74a71aCce4a2641';
...
// function getcurrentprice($crypto,$currency){
