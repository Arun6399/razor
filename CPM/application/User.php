...
class User extends CI_Controller {
   ...
   function forgot_user()
	...
	{		 
		...
		if($_POST)
		{
			...
			if($this->form_validation->run())
			{
				...
				$insertData['cpm_phone'] = $this->db->escape_str($this->input->post('mobile'));
				//$insertData['reference_currency'] = $this->db->escape_str($this->input->post('ref_currency'));
				//$insertData['street_address'] = $this->db->escape_str($this->input->post('address'));
				//$insertData['city'] = $this->db->escape_str($this->input->post('city'));
				//$insertData['state'] = $this->db->escape_str($this->input->post('state'));
	...
	function pwcheck(){
        $pwd = $_POST['oldpass'];
        $epwd = encryptIt($pwd);
        $Cnt_Row = $this->common_model->getTableData('users', array('cpm_password' => $epwd,'id'=>$this->session->userdata('user_id')))->num_rows();    
        if($Cnt_Row > 0){
            echo '0';
        }
        else{
           ...
           { 
			...
			if(!empty($tot_transactions) && count($tot_transactions)>0)
			{
				...
				foreach ($tot_transactions as $row) 
				{
					...
					$counts_tx = $this->db->query('select * from cpm_transactions where information="'.$row['blockhash'].'" and wallet_txid="'.$txid.'"')->num_rows();
...
front_redirect('block_ip');
		...
		if(isset($_POST['depist']))
		{
		    ...
		    $firstName = $getuser_details->cpm_fname;
		    $lastName = $getuser_details->cpm_lname;
    ...
    "firstName":"'.$userinfo->cpm_fname.'",
    "city":"'.$userinfo->city.'",
    "phone":"+'.$user_countries->phone_number.$userinfo->cpm_phone.'",
...
function phone_verification(){
	...
	$data['site_common'] = site_common();
        ...
        if(isset($_REQUEST['submitphone'])){
            ...
            'cpm_phone'=>$userst->phone_number."-".$number);
