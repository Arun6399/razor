 ...
 */
 class Wallet extends CI_Controller {
 	...
 	function wallet_ajax()
 	{
        ...
        // if(!empty($search))
        // { 
        //     $like = "AND cpm_users.cpm_username LIKE '%".$search."%'"; 
        // }
        if(!empty($search))
        { 
        	if(strlen($search) > '4') {
        		...
        		$like = " AND (users.cpm_email LIKE '%".$str1."%' AND history.cpm_type LIKE '%".encryptIt($str)."%')";
        	} 
        	if(strlen($search) <= '4') {
        		$str = substr($search, 0, strlen($search));
        		$like = " AND (users.cpm_email LIKE '%%' OR history.cpm_type LIKE '%".encryptIt($str)."%')";
        	}
            //$like = "AND cpm_users.cpm_email LIKE '%".$search."%'"; 
        }
        
         $query = "SELECT * FROM `cpm_users` AS users INNER JOIN cpm_history AS history INNER JOIN cpm_wallet AS wallet on users.id=history.user_id AND users.id=wallet.user_id WHERE wallet.status=1 ".$like." ORDER BY users.created_on DESC LIMIT ".$start.",".$length;
        $tt = $query;
		...
		//To Find Total
		$query_total = "SELECT * FROM `cpm_users` AS users INNER JOIN cpm_history AS history INNER JOIN cpm_wallet AS wallet on users.id=history.user_id AND users.id=wallet.user_id WHERE wallet.status=1 ".$like." ORDER BY users.created_on DESC";
