...
JOIN `cpm_users` as `c` ON `a`.`user_id`=`c`.`id` WHERE `a`.`type`="Deposit" AND `a`.`currency_type`="fiat" ORDER BY `a`.`trans_id` DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT a.*, b.currency_symbol as currency_symbol, b.currency_name as currency_name, b.currency_sign as currency_sign, c.cpm_username as username FROM `cpm_transactions` as `a` JOIN `cpm_currency` as `b` ON `a`.`currency_id`=`b`.`id` JOIN `cpm_users` as `c` ON `a`.`user_id`=`c`.`id` WHERE `a`.`type`="Deposit" AND `a`.`currency_type`="fiat" ORDER BY `a`.`trans_id` DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
...
$query = "SELECT a.*, b.currency_symbol as currency_symbol, b.currency_name as currency_name, b.currency_sign as currency_sign, c.cpm_username as username FROM cpm_transactions as a JOIN cpm_currency as b ON a.currency_id=b.id JOIN cpm_users as c ON a.user_id=c.id WHERE a.type='Deposit' AND a.currency_type='crypto'".$like." ORDER BY a.trans_id DESC LIMIT ".$start.",".$length;

$countquery = $this->db->query("SELECT a.*, b.currency_symbol as currency_symbol, b.currency_name as currency_name, b.currency_sign as currency_sign, c.cpm_username as username FROM cpm_transactions as a JOIN cpm_currency as b ON a.currency_id=b.id JOIN cpm_users as c ON a.user_id=c.id WHERE a.type='Deposit' AND a.currency_type='crypto'".$like." ORDER BY a.trans_id DESC");
        ...
        {
        	$query = 'SELECT a.*, b.currency_symbol as currency_symbol, b.currency_name as currency_name, b.currency_sign as currency_sign, c.cpm_username as username FROM `cpm_transactions` as `a` JOIN `cpm_currency` as `b` ON `a`.`currency_id`=`b`.`id`
JOIN `cpm_users` as `c` ON `a`.`user_id`=`c`.`id` WHERE `a`.`type`="Deposit" AND `a`.`currency_type`="crypto" ORDER BY `a`.`trans_id` DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT a.*, b.currency_symbol as currency_symbol, b.currency_name as currency_name, b.currency_sign as currency_sign, c.cpm_username as username FROM `cpm_transactions` as `a` JOIN `cpm_currency` as `b` ON `a`.`currency_id`=`b`.`id` JOIN `cpm_users` as `c` ON `a`.`user_id`=`c`.`id` WHERE `a`.`type`="Deposit" AND `a`.`currency_type`="crypto" ORDER BY `a`.`trans_id` DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
	...
	function index() {
		...
		$data['deposit'] = $this->common_model->getJoinedTableData('transactions as a',$hisjoins,$hiswhere,'a.*,b.currency_symbol as currency_symbol, b.currency_name as currency_name, b.currency_sign as currency_sign, c.cpm_username as username','','','','','',array('a.trans_id', 'DESC'))->result();
	...
	function view($id){
		...
		$data['deposit'] = $this->common_model->getJoinedTableData('transactions as a',$hisjoins,$hiswhere,'a.*,b.currency_symbol as currency_symbol, b.currency_name as currency_name, b.currency_sign as currency_sign, c.cpm_username as username','','','','','',array('a.trans_id', 'DESC'))->row();
		//echo $this->db->last_query();
	...
	function dash()
	{
		...
		$data['deposit'] = $this->common_model->getJoinedTableData('transactions as a',$hisjoins,$hiswhere,'a.*,b.currency_symbol as currency_symbol, b.currency_name as currency_name, c.cpm_username as username','','','','','',array('a.trans_id', 'DESC'))->result();
	...
	function crypto_view($id){
		...
		$data['deposit'] = $this->common_model->getJoinedTableData('transactions as a',$hisjoins,$hiswhere,'a.*,b.currency_symbol as currency_symbol, b.currency_name as currency_name, c.cpm_username as username','','','','','',array('a.trans_id', 'DESC'))->row();
		// echo '<pre>'; print_r($data['deposit']); die;
		$data['prefix'] = get_prefix();
