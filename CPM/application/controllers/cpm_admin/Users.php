 ...
 */
 class Users extends CI_Controller {
	...
	function users_ajax()
	{
        ...
        // if(!empty($search))
        // { 
        //     $like = "AND (cpm_users.cpm_username LIKE '%".$search."%' OR cpm_users.cpm_email LIKE '%".$search."%' OR cpm_history.cpm_type LIKE '%".$encrypt_search."%')"; 
        // }
        
        // $query = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=1 ".$like." ORDER BY created_on DESC LIMIT ".$start.",".$length;
        // $users_history = $this->db->query($query);
        // $users_history_result = $users_history->result(); 
        // $num_rows = count($users_history_result);

        // //To Find Total
        //  $query_total = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=1 ".$like." ORDER BY created_on DESC";
        // $users_history_total = $this->db->query($query_total);
        // $num_rows = $users_history_total->num_rows();

    if(!empty($search))
        { 
         //    $like = "WHERE (cpm_users.cpm_username LIKE '%".$search."%' OR cpm_users.cpm_email LIKE '%".$search."%' OR cpm_history.cpm_type LIKE '%".$encrypt_search."%')"; 
         //    $query = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id ".$like." ORDER BY cpm_users.".$order." ".$dir." LIMIT ".$start.",".$length;
	        // $countquery = $this->db->query("SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id ".$like." ORDER BY cpm_users.".$order." ".$dir."");
	        // $users_history = $this->db->query($query);
	        // $users_history_result = $users_history->result(); 
        ...
        $str1=$str[0];
        $str2=$str[1];
        $like = "WHERE (cpm_users.cpm_username LIKE '%".$search."%' OR cpm_users.id LIKE '%".$search."%' OR (cpm_users.cpm_email LIKE '%".$str2."%' AND cpm_history.cpm_type LIKE '%".encryptIt($str1)."%'))"; 
        $query = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id ".$like." ORDER BY cpm_users.".$order." ".$dir." LIMIT ".$start.",".$length;
        $countquery = $this->db->query("SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id ".$like." ORDER BY cpm_users.".$order." ".$dir."");
        $users_history = $this->db->query($query);
        $users_history_result = $users_history->result(); 
        ...
        {
        	$query = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id ORDER BY cpm_users.".$order." ".$dir." LIMIT ".$start.",".$length;
	        $countquery = $this->db->query("SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id ORDER BY cpm_users.".$order." ".$dir."");
	        $users_history = $this->db->query($query);
	        $users_history_result = $users_history->result(); 
	        $num_rows = $countquery->num_rows();
        }
		...
		if(count($users_history_result)>0)
		{
			foreach($users_history->result() as $users){
				$i++;
				// $fname = getUserDetails($users->user_id,'cpm_fname');
	...
	function unusers_ajax()
	{
        ...
        if(!empty($search))
        { 
            // $like = "AND (cpm_users.cpm_username LIKE '%".$search."%' OR cpm_users.unique_id LIKE '%".$search."%' OR cpm_users.cpm_email LIKE '%".$search."%' OR cpm_history.cpm_type LIKE '%".$encrypt_search."%')";
            $str=splitEmail($search);
        $str1=$str[0];
        $str2=$str[1];
        $like = "WHERE (cpm_users.cpm_username LIKE '%".$search."%' OR cpm_users.id LIKE '%".$search."%' OR (cpm_users.cpm_email LIKE '%".$str2."%' AND cpm_history.cpm_type LIKE '%".encryptIt($str1)."%'))"; 
        ...
        $query = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=0 ".$like." ORDER BY created_on DESC LIMIT ".$start.",".$length;
        $tt = $query;
        $users_history = $this->db->query($query);
        $users_history_result = $users_history->result(); 
		...
		//To Find Total
		$query_total = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=0 ".$like." ORDER BY created_on DESC";
		$users_history_total = $this->db->query($query_total);
        $num_rows = $users_history_total->num_rows();
  ...
  //       $query = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=1 AND verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC LIMIT ".$start.",".$length;
  //       $tt = $query;
  //       $users_history = $this->db->query($query);
  //       $users_history_result = $users_history->result(); 
  ...
  //       $query = "SELECT * FROM `cpm_users` INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=1 AND cpm_users.verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC LIMIT ".$start.",".$length;
  //       $tt = $query;
  //       $users_history = $this->db->query($query);
  //       $users_history_result = $users_history->result(); 
		...
		{
				$query = "SELECT * FROM cpm_users INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=1 AND cpm_users.verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC LIMIT ".$start.",".$length;
				$countquery = $this->db->query("SELECT * FROM cpm_users INNER JOIN cpm_history on cpm_users.id=cpm_history.user_id WHERE cpm_users.verified=1 AND cpm_users.verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC");
				$users_history = $this->db->query($query);
				$users_history_result = $users_history->result();
				$num_rows = $countquery->num_rows(); 
		}
   ...
   /*	$data['users'] = $this->common_model->getTableData('users','', '', '', '', '',$urisegment,$perpage,array('created_on', 'DESC'));*/
	...
	if(isset($_GET['search_string']) && !empty($_GET['search_string'])){
			...
			$like = array("cpm_username"=>$search_string);	
