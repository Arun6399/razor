			...
			if ($this->form_validation->run())
			{
				...
				if ($num_rows > 0) 
				{
					...
					if ($update) 
					{
						...
						$field_check = $this->db->query("SHOW COLUMNS FROM cpm_crypto_address LIKE '".$getaddcoin->currency_symbol."_status'");
						$field_exists = (count($field_check->result()))?TRUE:FALSE;

						if($field_exists == FALSE)
                          $this->db->query(" ALTER TABLE cpm_crypto_address ADD ".$getaddcoin->currency_symbol."_status INT(11) NOT NULL");

                          //ALTER TABLE `cpm_crypto_address` ADD `PPD_status` INT(11) NOT NULL AFTER `BUSD_status`; 
