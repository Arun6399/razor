 ...
 */
 class Trading_fee extends CI_Controller {
 	...
 	function tradingfee_ajax()
 	{
        ...
        if(!empty($search))
        { 
            $like = "WHERE (cpm_tradingfee.currency_name LIKE '%".$search."%' OR cpm_tradingfee.currency_symbol LIKE '%".$search."%')"; 
        }
        
        $query = "SELECT * FROM `cpm_tradingfee` ".$like." ORDER BY id DESC LIMIT ".$start.",".$length;
