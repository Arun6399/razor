...
$query = "SELECT * FROM `cpm_email_list`".$like." ORDER BY id DESC LIMIT ".$start.",".$length;
$countquery = $this->db->query("SELECT * FROM `cpm_email_list`".$like." ORDER BY id DESC");
        ...
        {
        	$query = 'SELECT * FROM `cpm_email_list` ORDER BY `id` DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT * FROM `cpm_email_list` ORDER BY `id` DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
	...
	{		
		...
		{				
			if ($this->form_validation->run())
			{
				...
				$details 							= $this->db->query("SELECT * FROM `cpm_currency` WHERE `currency_name` = '".$currency_name."' OR `currency_symbol` LIKE '%".$currency_symbol."'");
