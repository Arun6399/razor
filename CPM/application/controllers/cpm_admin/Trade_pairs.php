 ...
 class Trade_pairs extends CI_Controller {
 	...
 	function tradepairs_ajax()
 	{
        ...
        { 
            ...
            $query = 'SELECT a.*, b.currency_name as from_currency, b.currency_symbol as from_currency_symbol, c.currency_name as to_currency, c.currency_symbol as to_currency_symbol FROM `cpm_trade_pairs` as `a` JOIN `cpm_currency` as `b` ON `a`.`from_symbol_id` = `b`.`id` JOIN `cpm_currency` as `c` ON `a`.`to_symbol_id` = `c`.`id`'.$like." LIMIT ".$start.",".$length;

            $countquery = $this->db->query('SELECT a.*, b.currency_name as from_currency, b.currency_symbol as from_currency_symbol, c.currency_name as to_currency, c.currency_symbol as to_currency_symbol FROM `cpm_trade_pairs` as `a` JOIN `cpm_currency` as `b` ON `a`.`from_symbol_id` = `b`.`id` JOIN `cpm_currency` as `c` ON `a`.`to_symbol_id` = `c`.`id`'.$like);
        ...
        {
        	$query = 'SELECT a.*, b.currency_name as from_currency, b.currency_symbol as from_currency_symbol, c.currency_name as to_currency, c.currency_symbol as to_currency_symbol FROM `cpm_trade_pairs` as `a` JOIN `cpm_currency` as `b` ON `a`.`from_symbol_id` = `b`.`id` JOIN `cpm_currency` as `c` ON `a`.`to_symbol_id` = `c`.`id` LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT a.*, b.currency_name as from_currency, b.currency_symbol as from_currency_symbol, c.currency_name as to_currency, c.currency_symbol as to_currency_symbol FROM `cpm_trade_pairs` as `a` JOIN `cpm_currency` as `b` ON `a`.`from_symbol_id` = `b`.`id` JOIN `cpm_currency` as `c` ON `a`.`to_symbol_id` = `c`.`id`');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
