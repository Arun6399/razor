...
class Admin_wallet extends CI_Controller
{
    ...
    function wallet_ajax()
    {
        ...
        { 
            ...
            $query = "SELECT * FROM cpm_admin_wallet WHERE id=1".$like." LIMIT ".$start.",".$length;
            $countquery = $this->db->query("SELECT * FROM cpm_admin_wallet WHERE id=1".$like);
        ...
        {
            $query = 'SELECT * FROM cpm_admin_wallet WHERE id=1 LIMIT '.$start.','.$length;

            $countquery = $this->db->query('SELECT * FROM cpm_admin_wallet WHERE id=1');
            $users_history = $this->db->query($query);
            $users_history_result = $users_history->row(); 
            $get_admin_det = json_decode($users_history_result->addresses);
            $num_rows = count((array)$get_admin_det);         
        ...
        if($num_rows>0)
        {
            ...
            foreach ($get_admin_det as $key => $value) 
            {
               ...
               if($sym=="XRP" || $sym=="ETH"){
                    $withdraw_url = base_url() . "cpm_admin/admin_wallet/admin_withdraw_from_user/" . $sym;
               }else{
                    $withdraw_url = base_url() . "cpm_admin/admin_wallet/admin_withdraw/" . $sym;
               }
               $withdraw_url = base_url() . "cpm_admin/admin_wallet/admin_withdraw/" . $sym;
               if($sym !='USD')
               {
               ...
               $link = base_url()."cpm_admin/admin_wallet/admin_deposit/".$sym;
    ...
    public function admin_deposit($coinname)
    {
            ...
            $this->db->select('*')
                 ->from('cpm_wallet')
                 ->join('cpm_users', 'cpm_wallet.user_id = cpm_users.id');
                                ...
                                if ($insert) {
                                    ...
                                    if ($isValid > 0) {
                                        ...
                                        if ($withdraw->status == 'Completed') {
                                            ...
                                            admin_redirect('cpm_admin/admin_wallet', 'refresh');
