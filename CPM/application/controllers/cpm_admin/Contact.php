...
$query = "SELECT * FROM `cpm_contact_us`".$like." ORDER BY id DESC LIMIT ".$start.",".$length;

$countquery = $this->db->query("SELECT * FROM `cpm_contact_us`".$like." ORDER BY id DESC");
        ...
        {
        	$query = 'SELECT * FROM `cpm_contact_us` ORDER BY `id` DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT * FROM `cpm_contact_us` ORDER BY `id` DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
	...
	function edit($id) {
		...
		if ($this->input->post()) {			
			if ($this->form_validation->run())
			{
				...
				$update = $this->common_model->updateTableData('cpm_contact_us', $condition, $updateData);
				if ($update) {
