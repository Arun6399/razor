...
class Admin extends CI_Controller {
	...
	function dashboard_ajax()
	{
        ...
        if(!empty($search))
        { 
            $like = " WHERE b.cpm_email LIKE '%".$search."%' OR c.cpm_type LIKE '%".$encrypt_search."%' OR a.activity LIKE '%".$search."%' OR a.ip_address LIKE '%".$search."%'";

$query = "SELECT a.*,b.cpm_email,c.cpm_type FROM `cpm_user_activity` as a INNER JOIN cpm_users as b INNER JOIN cpm_history as c on a.user_id=b.id AND b.id=c.user_id ".$like." ORDER BY a.act_id DESC LIMIT ".$start.",".$length;

$countquery = $this->db->query("SELECT a.*,b.cpm_email,c.cpm_type FROM `cpm_user_activity` as a INNER JOIN cpm_users as b INNER JOIN cpm_history as c on a.user_id=b.id AND b.id=c.user_id".$like." ORDER BY a.act_id DESC");
        ...
        {
        	$query = 'SELECT a.*,b.cpm_email,c.cpm_type FROM `cpm_user_activity` as a INNER JOIN cpm_users as b INNER JOIN cpm_history as c on a.user_id=b.id AND b.id=c.user_id ORDER BY a.act_id DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT a.*,b.cpm_email,c.cpm_type FROM `cpm_user_activity` as a INNER JOIN cpm_users as b INNER JOIN cpm_history as c on a.user_id=b.id AND b.id=c.user_id ORDER BY a.act_id DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
	...
	{
		// $daily="SELECT SUM(`theftAmount`) as `total` FROM `cpm_coin_theft` where `date`='$date1' AND `theftCurrency`='$currency'";
		$daily="SELECT SUM(`profit_amount`) as `total` FROM `cpm_transaction_history` where datetime LIKE '%$date1%' AND `currency`='$currency'";
	...
	function coinprofit_ajax()
	{
        ...
        if(!empty($search))
        { 
            $like = " WHERE b.cpm_username LIKE '%".$search."%' OR a.type LIKE '%".$search."%' OR a.profit_amount LIKE '%".$search."%' OR c.currency_symbol LIKE '%".$search."%' OR a.comment LIKE '%".$search."%'";

$query = "SELECT a.*, b.cpm_username as username,c.currency_symbol FROM cpm_transaction_history as a JOIN cpm_users as b ON a.userId = b.id JOIN cpm_currency as c ON c.id = a.currency".$like." ORDER BY a.id DESC LIMIT ".$start.",".$length;


$countquery = $this->db->query("SELECT a.*, b.cpm_username as username,c.currency_symbol FROM cpm_transaction_history as a JOIN cpm_users as b ON a.userId = b.id JOIN cpm_currency as c ON c.id = a.currency".$like." ORDER BY a.id DESC");
        ...
        {
        	$query = 'SELECT a.*, b.cpm_username as username,c.currency_symbol FROM cpm_transaction_history as a JOIN cpm_users as b ON a.userId = b.id JOIN cpm_currency as c ON c.id = a.currency ORDER BY `a`.`id` DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT a.*, b.cpm_username as username,c.currency_symbol FROM cpm_transaction_history as a JOIN cpm_users as b ON a.userId = b.id JOIN cpm_currency as c ON c.id = a.currency ORDER BY `a`.`id` DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
	...
	function coin_profit_report($currency="1", $type="fiat")
	{
		...
		$daily="SELECT DATE(datetime) as dateval, SUM(profit_amount) as total, SUM(bonus_amount) as bonus FROM cpm_transaction_history where currency='$currency' AND currency_type='$type' GROUP BY YEAR(datetime), MONTH(datetime), DATE(datetime)";
		$data['daily']=$this->db->query($daily);

		$weekly="SELECT  WEEK(`datetime`) as week_number,YEAR(datetime) as yname,SUM(`profit_amount`) as total, SUM(bonus_amount) as bonus FROM `cpm_transaction_history` where currency='$currency' GROUP BY WEEK(`datetime`)";

		$data['weekly']=$this->db->query($weekly);


		$monthly="SELECT MONTHNAME(datetime) as moname,YEAR(datetime) as yname, SUM(profit_amount) as total, SUM(bonus_amount) as bonus FROM cpm_transaction_history where currency='$currency' GROUP BY YEAR(datetime), MONTH(datetime)";
		$data['monthly']=$this->db->query($monthly);


		$yearly="SELECT YEAR(datetime) as yname, SUM(profit_amount) as total, SUM(bonus_amount) as bonus FROM cpm_transaction_history where currency='$currency' GROUP BY YEAR(datetime)";
	...
	function coin_profit_report_ajax($currency="1", $type="fiat")
	{
		...
		$daily="SELECT DATE(datetime) as dateval, SUM(profit_amount) as total FROM cpm_transaction_history where currency='$currency' AND currency_type='$type' GROUP BY YEAR(datetime), MONTH(datetime), DATE(datetime)";
		$data['daily']=$this->db->query($daily);
		...
		$weekly="SELECT  WEEK(`datetime`) as week_number,YEAR(datetime) as yname,SUM(`profit_amount`) as total FROM `cpm_transaction_history` where currency='$currency' AND currency_type='$type' GROUP BY WEEK(`datetime`)";
		...
		$monthly="SELECT MONTHNAME(datetime) as moname,YEAR(datetime) as yname, SUM(profit_amount) as total FROM cpm_transaction_history where currency='$currency' AND currency_type='$type' GROUP BY YEAR(datetime), MONTH(datetime)";
		$data['monthly']=$this->db->query($monthly);
		...
		$yearly="SELECT YEAR(datetime) as yname, SUM(profit_amount) as total FROM cpm_transaction_history where currency='$currency' AND currency_type='$type' GROUP BY YEAR(datetime)";
	...
	function ip_ajax()
	{
        ...
        { 
			...
			$query = "SELECT * FROM cpm_page_handling".$like." ORDER BY id DESC LIMIT ".$start.",".$length;


			$countquery = $this->db->query("SELECT * FROM cpm_page_handling".$like." ORDER BY id DESC");
        ...
        {
        	$query = 'SELECT * FROM cpm_page_handling ORDER BY id DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT * FROM cpm_page_handling ORDER BY id DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
	...
	function users_transferbalance()
	{
		...
		if(isset($_GET['search_string']) && !empty($_GET['search_string'])){
			...
			$like_or = array("c.cpm_username"=>$search_string, "b.currency_name"=>$search_string, "a.amount"=>$search_string, "a.from_account"=>$search_string, "a.to_account"=>$search_string);
			$hisjoins = array('currency as b'=>'a.currency = b.id','users as c'=>'a.user_id = c.id');
			$data['transfers'] = $this->common_model->getJoinedTableDatas('transfer_history as a',$hisjoins,'','a.*,b.currency_symbol as currency_symbol, b.currency_name as currency_name, c.cpm_username as username',$like,'',$like_or,'','',array('a.id', 'DESC'));	
		}
		else {
   		$perpage = max_records();
  		$urisegment=$this->uri->segment(4);  
		...
		$hisjoins = array('currency as b'=>'a.currency = b.id','users as c'=>'a.user_id = c.id');
		$data['transfers'] = $this->common_model->getJoinedTableData('transfer_history as a',$hisjoins,'','a.*,b.currency_symbol as currency_symbol, b.currency_name as currency_name, c.cpm_username as username','','','',$urisegment,$perpage,array('a.id', 'DESC'));
		}
    ...
    function userbank_ajax()
    {
        ...
        if(!empty($search))
        { 
            $like = " WHERE c.cpm_username LIKE '%".$search."%' OR b.currency_symbol LIKE '%".$search."%' OR a.bank_name LIKE '%".$search."%' OR a.bank_account_number LIKE '%".$search."%' OR a.bank_account_name LIKE '%".$search."%' OR a.status LIKE '%".$st."%'";

$query = "SELECT a.*, c.cpm_username as username,b.currency_symbol FROM cpm_user_bank_details as a JOIN cpm_users as c ON a.user_id = c.id JOIN cpm_currency as b ON b.id = a.currency".$like." ORDER BY a.id DESC LIMIT ".$start.",".$length;


$countquery = $this->db->query("SELECT a.*, c.cpm_username as username,b.currency_symbol FROM cpm_user_bank_details as a JOIN cpm_users as c ON a.user_id = c.id JOIN cpm_currency as b ON b.id = a.currency".$like." ORDER BY a.id DESC");
        ...
        {
        	$query = 'SELECT a.*, c.cpm_username as username,b.currency_symbol FROM cpm_user_bank_details as a JOIN cpm_users as c ON a.user_id = c.id JOIN cpm_currency as b ON b.id = a.currency ORDER BY a.id DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT a.*, c.cpm_username as username,b.currency_symbol FROM cpm_user_bank_details as a JOIN cpm_users as c ON a.user_id = c.id JOIN cpm_currency as b ON b.id = a.currency ORDER BY a.id DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
	...
	function user_bank_status($id, $status) {
		...
		if($isValid > 0)
		{
			...
			{		
                ...
                {
                  $username = $userdetail->cpm_username;
                }
                else
                {
