	    		...
	    		if(empty($User_Address) || $User_Address==0)
	    		{
		            ...
		            else
		            {
		            	...
		            	if($coin_address->currency_symbol=='CPM'){
		            		...
		            		$model_name = 'Token_bnb_wallet_model';
							$model_location = 'wallets/'.$model_name;           
							$this->load->model($model_location,$model_name);

		            		$Get_First_address = $this->$model_name->create_eth_account(getUserEmail($user_details->user_id));
...
public function coinpayments_crypto_deposit($curr,$crypto_type)
	{
		...
		if($valid_server)
		{
			...
			if($coin_type=="coin")
			{
				...
				$get_deposit = $this->$model_name->listalltransactions($user_trans_res,$coin_sym);
