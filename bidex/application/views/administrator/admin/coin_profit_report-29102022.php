                            ...
                            <div class="table-responsive">
								...
								if ($weekly->num_rows() > 0) {
									$i = 1;
										...
										echo '<tr><td>1</td><td>36</td><td>2022</td><td>' . $btc . '</td></tr>';
										// echo '<tr><td>1</td><td>40</td><td>2022</td><td>' . $btc1 . '</td></tr>';

									// echo "<pre>";print_r(array_reverse($weekly->result()));	
									foreach(array_reverse($weekly->result()) as $key => $result1) {
									  if($result1->week_number > 30) {

										$bonus=$result1->total+$result1->bonus;

										if($currency_symbol == 'BTC'){ 
											if($key==0) $amt = 0.06172; 
											else if($key==1) $amt = 0.105418;
											else if($key==2) $amt = 0.158127;
											else if($key==3) $amt = 0.158127;
											else if($key==4) $amt = 0.12;
											else if($key==5) $amt = 0.210836;

										} if($currency_symbol == 'ETH'){ 
											// if($result1->week_number!='36') {

											if($key==0) $amt = 1.7649; 
											else if($key==1) $amt = 0.0757;
											else if($key==2) $amt = 0.0606;
											else if($key==3) $amt = 0.2424;
											else if($key==4) $amt = 0.1212;
											else if($key==5) $amt = 0.590311;
											// else if($key==6) $amt = 0.0757;
											// }
