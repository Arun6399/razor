...
//$user_id = 11;
		...
		else
		{
				...
				$newpassword= $this->db->escape_str($this->input->post('newpassword'));
				...
				// $insertData['bidex_cpassword']=encryptIt($newpassword);
					...
					$insertData = array('bidex_fname'=>$fname,
                                        'bidex_lname'=>$lname,
                                        'bidex_phone'=>$phone,
                                        'bidex_password'=>encryptIt($password),
                                        'bidex_cpassword'=>encryptIt($newpassword),
	...
	function address_verification_app()	{
		...
		{	
            ...
            $image = $_FILES['photo_id_1']['name'];
                ...
                {
                    $image = $this->input->post('photo_id_1');
                }
            ...
            $image2 = $_FILES['photo_id_2']['name'];
                if($image2!="")
                {        
                    $Img_Size2 = $_FILES['photo_id_2']['size'];
                    if($Img_Size2>2000000){
                        $data['status']='0';
                        $data['msg'] = 'Back Address Proof Should be less than 2 MB';
                    }
                    $uploadimage2=cdn_file_upload($_FILES["photo_id_2"],'uploads/user/'.$user_id,$this->db->escape_str($this->input->post('photo_id_2')));
                    if($uploadimage2)
                    {
                ...
                {
                    $image2 = $this->input->post('photo_id_2');
                }
                
                $insertData = array();
                $insertData['photo_id_1'] = $image;
                $insertData['photo_id_2'] = $image2;                    
                $insertData['verify_level2_date'] = gmdate(time());
                $insertData['verify_level2_status'] = 'Pending';
                $insertData['photo_1_status'] = 1;
                $insertData['photo_2_status'] = 1;                    
                ...
                if($insert2) {
                    $data['status']='1';
                    $data['msg'] = 'Your Address Proof details have been sent to our team for verification';
                }
                else {
...
$data=array();
     ...
     $image3 = $_FILES['photo_id_3']['name'];
                if($image3!="")
                {        
                    $Img_Size3 = $_FILES['photo_id_3']['size'];
                    if($Img_Size3>2000000){
                        $data['status']='0';
                        $data['msg'] = 'Front Id Proof Should be less than 2 MB';
                    }
                    $uploadimage3=cdn_file_upload($_FILES["photo_id_3"],'uploads/user/'.$user_id,$this->db->escape_str($this->input->post('photo_id_3')));
                    if($uploadimage3)
                    {
                ...
                {
                    $image3 = $this->db->escape_str($this->input->post('photo_id_3'));
                }
            ...
            $image4 = $_FILES['photo_id_4']['name'];
                if($image4!="")
                {        
                    $Img_Size4 = $_FILES['photo_id_4']['size'];
                    if($Img_Size4>2000000){
                        $data['status']='0';
                        $data['msg'] = 'Back Id Proof Should be less than 2 MB';
                    }
                    $uploadimage4=cdn_file_upload($_FILES["photo_id_4"],'uploads/user/'.$user_id,$this->db->escape_str($this->input->post('photo_id_4')));
                    if($uploadimage4)
                    {
                ...
                {
                    $image4 = $this->db->escape_str($this->input->post('photo_id_4'));
                }
                
                $insertData = array();
                $insertData['photo_id_3'] = $image3;
                $insertData['photo_id_4'] = $image4;                    
                $insertData['verify_level2_date'] = gmdate(time());
                $insertData['verify_level2_status'] = 'Pending';
                $insertData['photo_3_status'] = 1;  
                $insertData['photo_4_status'] = 1;                    
	...
	function photo_verification_app()	{
		...
		if($_FILES['photo_id_5'])    {
            ...
            $image5 = $_FILES['photo_id_5']['name'];
                ...
                elseif($this->input->post('photo_ids_5')=='')
                {
                    $image5 = $this->db->escape_str($this->input->post('photo_ids_5'));
                }
                else 
                { 
                    $image5='';
...
$from_currency = $this->common_model->getTableData('currency',array('id' => $Currency_list->currency_id))->row();
			...
			$Currency_list_Val[$i] = array(	"transaction_id"=>$Currency_list->trans_id,
												"datetime"=>$Currency_list->datetime,
			...
			if($get_users->randcode == 'enable'){
			   ...
			   $randcode = '0';
		 		...
		 		if($get_users->photo_1_status==1 && $get_users->photo_2_status==1 && $get_users->photo_3_status==1 && $get_users->photo_4_status==1 && $get_users->photo_5_status==1){
			       ...
			       }elseif($get_users->photo_1_status==2 && $get_users->photo_2_status==2 && $get_users->photo_3_status==2 && $get_users->photo_4_status==2 && $get_users->photo_5_status==2) {
			       	$verify_status = "Rejected";
			       }elseif($get_users->photo_1_status==3 && $get_users->photo_2_status==3 && $get_users->photo_3_status==3 && $get_users->photo_4_status==3 && $get_users->photo_5_status==3) {
   ...
   return $res_data;
		...
		function close_active_order()
	{
		...
		if(!isset($user_id) && empty($user_id)){
			...
			$response['status'] = 0;
			$response['msg'] = "User ID cannot be empty";
		}
		else if(!isset($tradeid) && empty($tradeid)){

			$response['status'] = 0;
			$response['msg'] = "Trade ID cannot be empty";
		}else{
		// $response=
			// $cancel = $this->site_api->close_active_order($tradeid,$pair_id,$user_id);
			// print_r($cancel);
		if($this->site_api->close_active_order($tradeid,$pair_id,$user_id)){
			$response['status'] = 1;
			$response['msg'] = "Order Cancelled successfully";
		}else{
			$response['status'] = 0;
			$response['msg'] = "Order Cancelled failed";
		}
	}
		echo json_encode($response);
	}
