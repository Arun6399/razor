...
class Coin_request extends CI_Controller 
{
	...
	function index() 
	{
		...
		$data['coin_request'] 		= $this->common_model->getJoinedTableData('add_coin as a',$hisjoins,'','a.*, b.bidex_username as username','','','',$urisegment,$perpage,array('a.coin_id', 'DESC'));
	...
	function view($id) 
	{
		...
		$isValid 	= $this->common_model->getJoinedTableData('add_coin as a',$hisjoins,$hiswhere,'a.*, b.bidex_username as username');
	...
	function coin_request_ajax()
	{
		...
		$coin_requests = $this->db->get("bidex_add_coin");
		...
		if($num_rows > 0)
		{
			...
			{
				$i++;	           
                $view = '<a href="' . admin_url() . 'coin_request/view/' . $coin_request->coin_id . '" title="View this Request"><i class="fa fa-pencil text-primary"></i></a>&nbsp;&nbsp;&nbsp;';	          
				$data[] = array(
				    $i,
