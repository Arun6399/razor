 ...
 class Airdrops extends CI_Controller {
 	...
 	function airdrops_ajax()
 	{
        ...
        { 
			...
			$query = "SELECT * FROM bidex_airdrops ".$like." ORDER BY id DESC LIMIT ".$start.",".$length;
			$countquery = $this->db->query("SELECT * FROM bidex_airdrops ".$like." ORDER BY id DESC");
        ...
        {
        	$query = 'SELECT * FROM bidex_airdrops ORDER BY id LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT * FROM bidex_airdrops ORDER BY id DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
