 ...
 */
 class Users extends CI_Controller {
	...
	function users_ajax()
	{
        ...
        // if(!empty($search))
        // { 
        //     $like = "AND (bidex_users.bidex_username LIKE '%".$search."%' OR bidex_users.bidex_email LIKE '%".$search."%' OR bidex_history.bidex_type LIKE '%".$encrypt_search."%')"; 
        // }
        
        // $query = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 ".$like." ORDER BY created_on DESC LIMIT ".$start.",".$length;
        // $users_history = $this->db->query($query);
        // $users_history_result = $users_history->result(); 
        // $num_rows = count($users_history_result);

        // //To Find Total
        //  $query_total = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 ".$like." ORDER BY created_on DESC";
        // $users_history_total = $this->db->query($query_total);
        // $num_rows = $users_history_total->num_rows();

    if(!empty($search))
        { 
            $like = "WHERE (bidex_users.bidex_fname LIKE '%".$search."%' OR bidex_users.bidex_email LIKE '%".$search."%' OR bidex_history.bidex_type LIKE '%".$encrypt_search."%')"; 
            $query = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id ".$like." ORDER BY bidex_users.".$order." ".$dir." LIMIT ".$start.",".$length;
	        $countquery = $this->db->query("SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id ".$like." ORDER BY bidex_users.".$order." ".$dir."");
	        $users_history = $this->db->query($query);
	        $users_history_result = $users_history->result(); 
        ...
        {
        	$query = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id ORDER BY bidex_users.".$order." ".$dir." LIMIT ".$start.",".$length;
	        $countquery = $this->db->query("SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id ORDER BY bidex_users.".$order." ".$dir."");
	        $users_history = $this->db->query($query);
	        $users_history_result = $users_history->result(); 
	        $num_rows = $countquery->num_rows();
        }
	...
	function unusers_ajax()
	{
        ...
        if(!empty($search))
        { 
            $like = "AND (bidex_users.bidex_fname LIKE '%".$search."%' OR bidex_users.bidex_email LIKE '%".$search."%' OR bidex_history.bidex_type LIKE '%".$encrypt_search."%')"; 
        }
        
        $query = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=0 ".$like." ORDER BY created_on DESC LIMIT ".$start.",".$length;
        $tt = $query;
        $users_history = $this->db->query($query);
        $users_history_result = $users_history->result(); 
		...
		//To Find Total
		$query_total = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=0 ".$like." ORDER BY created_on DESC";
		$users_history_total = $this->db->query($query_total);
        $num_rows = $users_history_total->num_rows();
  ...
  //       $query = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 AND verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC LIMIT ".$start.",".$length;
  //       $tt = $query;
  //       $users_history = $this->db->query($query);
  //       $users_history_result = $users_history->result(); 
  ...
  //       // exit();
        ...
        $query = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 AND bidex_users.verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC LIMIT ".$start.",".$length;
        $tt = $query;
        $users_history = $this->db->query($query);
        $users_history_result = $users_history->result(); 
		...
		{
				$query = "SELECT * FROM bidex_users INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 AND bidex_users.verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC LIMIT ".$start.",".$length;
				$countquery = $this->db->query("SELECT * FROM bidex_users INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 AND bidex_users.verify_level2_status!='' ".$like." ORDER BY verify_level2_date DESC");
				$users_history = $this->db->query($query);
				$users_history_result = $users_history->result();
				$num_rows = $countquery->num_rows(); 
		}
		...
		if(count($users_history_result)>0)
		{
			foreach($users_history->result() as $users){
				$i++;
				$email = getUserEmail($users->user_id);
	            if($users->photo_1_status==3 && $users->photo_2_status==3 && $users->photo_3_status==3 && $users->photo_4_status==3 && $users->photo_5_status==3)
	            {
	                $level = '2';
	                $status = 'Level-'.$level.'&nbsp;&nbsp;'.'<label class="label label-info">Verified</label>';
	                
	            }
	            else
	            {
	                $level = '2';
	                if($users->photo_1_status==2 && $users->photo_2_status==2 && $users->photo_3_status==2 && $users->photo_4_status==2 && $users->photo_5_status==2)
	                {
	                    $status = 'Level-'.$level.'&nbsp;&nbsp;'.'<label class="label label-info">Rejected</label>';
	                    
	                }
	                else
	                {
	                  $status = 'Level-'.$level.'&nbsp;&nbsp;'.'<label class="label label-primary">Pending</label>'; 
	                }
	            }
   ...
   /*	$data['users'] = $this->common_model->getTableData('users','', '', '', '', '',$urisegment,$perpage,array('created_on', 'DESC'));*/
	...
	function verify_photo1_status($id,$status) {
		...
		if ($isValid > 0) { 
			...
			if ($update) { 
				...
				if($user_det->photo_1_status==3 && $user_det->photo_2_status==3 && $user_det->photo_3_status==3 && $user_det->photo_4_status==3 && $user_det->photo_5_status==3)
				{
			        ...
			        $kyc_status = $this->common_model->getTableData('users', array('id'=>$user_det->id,'photo_1_status'=>'3','photo_2_status'=>'3','photo_3_status'=>'3','photo_4_status'=>'3','photo_5_status'=>'3'))->row();
			        if($kyc_status)
			        {
		...
		if ($isValid > 0) { 
			...
			if ($update) { 
				$users = $isValids->row();
				$user_det = $this->common_model->getTableData('users', array('id' => $id))->row();
				if($user_det->photo_1_status==2 && $user_det->photo_2_status==2 && $user_det->photo_3_status==2 && $user_det->photo_4_status==2 && $user_det->photo_5_status==2)
				{
					$cond = array('id' => $user_det->id);
	...
	function verify_photo4_status($id,$status) {
		// Is logged in
		$sessionvar=$this->session->userdata('loggeduser');
		if (!$sessionvar) {
			admin_redirect('admin', 'refresh');
		...
		if ($isValid > 0) { 
			...
			if ($update) { 
				...
				$this->email_model->sendMail($email, '', '', $email_template,$special_vars);
				$this->session->set_flashdata('success', 'User photo id 4 verification successfully Completed');
				
				admin_redirect('users/verification_view/'.$id);
			} else { 
				$this->session->set_flashdata('error', 'Problem occure with user photo id 4 verification');
				admin_redirect('users/verification_view/'.$id);	
			}
		} else {
			$this->session->set_flashdata('error', 'Unable to find this user');
	...
	function verify_photo4_reject($id,$status) {
		// Is logged in
		$sessionvar=$this->session->userdata('loggeduser');
		if (!$sessionvar) {
			admin_redirect('admin', 'refresh');
		...
		if ($isValid > 0) { 
			...
			if ($update) { 
				...
				$this->email_model->sendMail($email, '', '', $email_template,$special_vars);
				$this->session->set_flashdata('success', 'User photo id 4 verification successfully Rejected');
	...
	function verify_photo5_status($id,$status) {
		// Is logged in
		$sessionvar=$this->session->userdata('loggeduser');
		if (!$sessionvar) {
			admin_redirect('admin', 'refresh');
		...
		if ($isValid > 0) { 
			$condition = array('id' => $id);
			$updateData['photo_5_status'] = $status;
			$update = $this->common_model->updateTableData('users', $condition, $updateData);
			if ($update) { 
				...
				$this->email_model->sendMail($email, '', '', $email_template,$special_vars);
				$this->session->set_flashdata('success', 'User photo id 5 verification successfully Completed');
				
				admin_redirect('users/verification_view/'.$id);
			} else { 
				$this->session->set_flashdata('error', 'Problem occure with user photo id 5 verification');
				admin_redirect('users/verification_view/'.$id);	
			}
		} else {
			$this->session->set_flashdata('error', 'Unable to find this user');
	...
	function verify_photo5_reject($id,$status) {
		// Is logged in
		$sessionvar=$this->session->userdata('loggeduser');
		if (!$sessionvar) {
			admin_redirect('admin', 'refresh');
		...
		if ($isValid > 0) { 
			$condition = array('id' => $id);
			$updateData['photo_5_status'] = $status;
			$updateData['photo_5_reason'] = $this->input->post('reject_mail_content');
			$update = $this->common_model->updateTableData('users', $condition, $updateData);
			if ($update) { 
				$users = $isValids->row();
				$user_det = $this->common_model->getTableData('users', array('id' => $id))->row();
				...
				$this->email_model->sendMail($email, '', '', $email_template,$special_vars);
				$this->session->set_flashdata('success', 'User photo id 5 verification successfully Rejected');
	...
	if(isset($_GET['search_string']) && !empty($_GET['search_string'])){
			...
			$like = array("bidex_username"=>$search_string);	
