 ...
 */
 class Users extends CI_Controller {
  ...
  //       if(!empty($search))
  //       { 
  //           $like = "AND (bidex_users.bidex_username LIKE '%".$search."%' OR bidex_users.bidex_email LIKE '%".$search."%' OR bidex_history.zonexh_type LIKE '%".$encrypt_search."%')"; 
  //       }
  ...
  //       $users_history_result = $users_history->result(); 
    ...
    else
    {
        ...
        $query = "SELECT * FROM `bidex_users` WHERE bidex_users.verified=1 ORDER BY id ASC LIMIT ".$start.",".$length;

        $countquery = $this->db->query("SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 ORDER BY bidex_users.id ASC");
        $users_history = $this->db->query($query);
        $users_history_result = $users_history->result(); 
...
{
$like = "AND (bidex_users.bidex_username LIKE '%".$search."%' OR bidex_users.bidex_email LIKE '%".$search."%' OR bidex_history.bidex_type LIKE '%".$encrypt_search."%')";
$query = "SELECT *,bidex_users.bidex_username FROM bidex_users INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=0 ".$like." ORDER BY bidex_users.id DESC LIMIT ".$start.",".$length;
$countquery = $this->db->query("SELECT * FROM bidex_users INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=0 ".$like." ORDER BY bidex_users.id DESC");
$users_history = $this->db->query($query);
$users_history_result = $users_history->result();
$num_rows = $countquery->num_rows();
}
...
{
$query = "SELECT *,bidex_users.bidex_username FROM bidex_users INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=0 ".$like." ORDER BY bidex_users.id DESC LIMIT ".$start.",".$length;
$countquery = $this->db->query("SELECT * FROM bidex_users INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=0 ORDER BY bidex_users.id DESC");
$users_history = $this->db->query($query);
$users_history_result = $users_history->result();
$num_rows = $countquery->num_rows();
}
	...
	function kyc_ajax()
	{
        ...
        $query = "SELECT * FROM `bidex_users` INNER JOIN bidex_history on bidex_users.id=bidex_history.user_id WHERE bidex_users.verified=1 AND bidex_users.verify_level2_status!='' ".$like." ORDER BY bidex_users.verify_level2_date DESC LIMIT ".$start.",".$length;
   ...
   /*	$data['users'] = $this->common_model->getTableData('users','', '', '', '', '',$urisegment,$perpage,array('created_on', 'DESC'));*/
	...
	if(isset($_GET['search_string']) && !empty($_GET['search_string'])){
			...
			$like = array("zonexh_username"=>$search_string);	
