...
$query = "SELECT * FROM `bidex_meta_content`".$like." ORDER BY id DESC LIMIT ".$start.",".$length;
$countquery = $this->db->query("SELECT * FROM `bidex_meta_content`".$like." ORDER BY id DESC");
        ...
        {
        	$query = 'SELECT * FROM `bidex_meta_content` ORDER BY `id` DESC LIMIT '.$start.','.$length;

        	$countquery = $this->db->query('SELECT * FROM `bidex_meta_content` ORDER BY `id` DESC');
        	$users_history = $this->db->query($query);
            $users_history_result = $users_history->result(); 
            $num_rows = $countquery->num_rows();            
        }
