...
function listalltransactions($user_trans_res, $contract_address='') 
	{
		...
		foreach ($address_list as $key => $value) {
			...
			$get_trans_url = 'https://api.etherscan.io/api?module=account&action=tokentx&contractaddress='.$contract_address.'&address='.$address.'&page=1&offset=100&sort=asc&apikey=61DI1HZQG7VURAGH29S14FMJEA3ZKKVA7A';
...
function isChecksumAddress($address) {
	...
	private function ether_request($cmd, $postfields=null) {
		...
		$ch = curl_init();
		//curl_setopt($ch, CURLOPT_URL, 'http://'.$this->wallet_ip.":".$this->wallet_port);
		curl_setopt($ch, CURLOPT_URL, 'http://142.132.252.120:8585');
		curl_setopt($ch, CURLOPT_USERPWD, $this->username.":".$this->password);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_ANY);
		//curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
	...
	$result = $this->ether_request('personal_newAccount', array("znB4qHnhCYspTtte2FB98J7VwaLS2yGd")); //personal_unlockAccount
		return $result;
