	...
	*/
	...
	function eth_getBalance($address,$coin_symbol,$contract_address, $block='latest', $decode_hex=FALSE)
	{
		   /* $tables = wallet_table();
			...
			$address  = $address;
		$get_trans_url='https://api.bscscan.com/api?module=account&action=tokenbalance&contractaddress='.$contract_address.'&address='.$address.'&tag=latest';
...
$ch = curl_init();
	...
	//echo '<pre>';print_r($trans);
		...
		$coin_detail = getcoindetail($coin_symbol);
		// echo "<pre>";
		// print_r($coin_detail); exit;
		if($coin_detail->bsc_currency_decimal != '')
		{
			$coin_decimal = $coin_detail->bsc_currency_decimal;
			$decimal_places = coin_decimal($coin_decimal);
		} 
		else if($coin_detail->currency_decimal != '')
		{
			$coin_decimal = $coin_detail->currency_decimal;
			$decimal_places = coin_decimal($coin_decimal);
		} else {
...
function listalltransactions($user_trans_res, $contract_address='') 
	{
		...
		foreach ($address_list as $key => $value) {
			...
			$get_trans_url = 'https://api.bscscan.com/api?module=account&action=tokentx&contractaddress='.$contract_address.'&address='.$address.'&page=1&offset=100&sort=asc&apikey=ZHKM64IMB5NT5ZAVJR7JS1988W6BKG9FVD';
            /*echo $get_trans_url;
            echo "<br>";*/
			//$trans = json_decode(file_get_contents($get_trans_url));
			$ch = curl_init();
...
if($ret1 !== FALSE)
   ...
   /* function toWei(float $value, int $decimals = 18)
	...
	$result = $this->ether_request('personal_newAccount', array("arP6yC2WJQGqjVKV6MdzQva9Y3agnj9Y")); //personal_unlockAccount
		return $result;
