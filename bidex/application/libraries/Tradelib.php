				...
				if($updatequery)
				{
					...
					if($dup_coin_order==0)
					{
						...
						if($user->bidex_email != '')
						{
							...
							// $data = ['phone' => '+'.$phcode.$number, 'text' => "smdex:Your ".ucfirst($type)." order of ".number_format($amount,8)." ".$currency_name." is confirmed by our team "];
...
function initialize_mapping($res)
{
	...
	if($buy)
	{
		...
		if($buy->Type=='buy')
		{
			...
			$pair_details = $this->ci->common_model->getTableData('trade_pairs',array('id'=>$pair),'from_symbol_id,to_symbol_id,api_status')->row();
			...
			// 		'Price' =>$Binance['price']
			// 		);
			// 		$this->ci->common_model->updateTableData('coin_order', array('trade_id' => $buyorderId), $updateTableData);*/
		...
		else if($buy->Type=='sell')
		{
            ...
            //   'Price' =>$Binance['price']
            // );
            // $this->ci->common_model->updateTableData('coin_order', array('trade_id' => $sellorderId), $updateTableData);*/
...
function ordercompletetype($orderId,$type,$inserted)
{
	...
	$user_data = $this->ci->common_model->getTableData('users',array('id'=>$user_id))->row();
	if($user_data->smd_balance > 0)
	{
		...
		$this->ci->common_model->updateTableData('users', array('id' => $user_id), $updateTableData);
	}
...
function close_active_order($tradeid,$pair_id,$user_id)
{
	...
	{    
		if($order->status=='active'||$order->status=='partially'||$order->status=='stoporder'||$order->status=='margin'||($order->status=='filled'&&$order->wallet=='Margin Trading'))
		{
			...
			//Binance _check 
		//Pilaventhiran 21/01/2021 START
					...
					else if($Type=="sell")
					{
						...
						{  
							...
							{ 
								...
								// if(checkMarketingUser($userId) == '1')
								// {
								// 	$updatesellBalance1 = $userbalance+$activeCalcTotal;
								// } else {
